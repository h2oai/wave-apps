...
async def test_sum_result(input: int, expected: int):
    assert await calculate_sum(input, on_update=on_update_pass, sleep=0.0001) == str(
        expected
    )
...
async def test_cancel_sum():
    ...
    await calculate_sum(5, on_update=on_update, sleep=0.0001)
...
async def test_updates_called(input):
    ...
    await calculate_sum(input, on_update=on_update, sleep=0.0001)
