...
def load_data_from_db():
    '''
    The function connects to Delta Lake and returns two Pandas DFs:
        - Employee Predictions
        - Shapley Values
    The function connects to Delta Lake by obraing connection parameters from Env Variables:
        - DATABRICKS_SERVER_HOSTNAME - obtain from DB cluster -> Advanced Options -> JDBC/ODBC -> Server Host Name
        - DATABRICKS_HTTP_PATH       - obtain from DB cluster -> Advanced Options -> JDBC/ODBC -> HTTP Path
        - DATABRICKS_TOKEN   - obtain from User -> User Settings -> Generate New Token
    :return:
        Two Pandas DFs
    '''
